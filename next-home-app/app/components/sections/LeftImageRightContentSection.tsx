...
export default function LeftImageRightContentSection({ data }: any) {
  ...
  height={400}  />
        ...
        <div className="lg:w-1/2">
          {meta?.badge && <span className="text-sm text-emerald-600 font-semibold">{meta.badge}</span>}
          <h2 className="text-5xl font-bold mt-4">{title}</h2>
          {sub_title && <p className="text-xl text-gray-600 mt-4">{sub_title}</p>}
          {meta?.content && <div className="mt-4 text-gray-700" dangerouslySetInnerHTML={{ __html: meta.content }} />}
          {meta?.cta?.length > 0 && (
            <div className="flex gap-4 mt-8">
              {meta.cta.map((btn: any, i: number) => <Button key={i}>{btn.label}</Button>)}
            </div>
          )}
        </div>
      </div>
